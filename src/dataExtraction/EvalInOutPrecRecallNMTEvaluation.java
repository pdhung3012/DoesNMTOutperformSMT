...
public class EvalInOutPrecRecallNMTEvaluation {
...
//			}
			...
			for(int j=0;j<((itemSource.length<=itemTrans.length)?itemSource.length:itemTrans.length);j++){
				if((!itemTarget[j].equals(itemSource[j]))&&(!itemSource[j].equals(tokenUnknown))){
				if(!setVocabTrainSource.contains(itemSource[j])){
						numCSourceLine++;
